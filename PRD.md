...
Игрок видит поле 6×6 букв и текущее целевое слово над сеткой.
...
Рекомендуемый каркас генерации уровня:

около 35% коротких лёгких слов,

около 35% средних слов средней сложности,

около 30% длинных сложных слов.

Это не жёсткий инвариант, а рекомендуемое распределение.

Если точное соблюдение мешает читаемости или стабильной генерации, допускаются небольшие отклонения.

Короткое слово = 3..4 буквы.

Среднее слово = 5..6 букв.

Длинное слово = 7+ букв.

Как минимум 30% target-слов уровня должны быть длинными (округление вверх).

Короткие слова допустимы как filler, но не должны визуально и количественно доминировать в уровне.
...
Выбираем набор целевых слов в диапазоне 10..15 с обязательной квотой длинных слов не ниже 30%.

Пытаемся уложить их в 6×6 так, чтобы уровень был не только формально валидным, но и визуально читаемым для поиска конкретного слова.
...
если после локальных замен не удаётся получить читаемый уровень с 10..15 словами и квотой длинных слов — отбрасываем набор целиком.
...
уровень не набирает диапазон 10..15 читаемых target-слов,

не выполнена квота длинных слов,

короткие слова визуально доминируют над средними/длинными.
...
Generator scaffold по сложности:

низкий rank трактуется как более частотное / более простое слово,

высокий rank трактуется как менее частотное / более сложное слово.

Для генерации уровня `rank` используется как мягкий сигнал при сборке каркаса `35 / 35 / 30`, а не как жёсткий numeric cutoff, зашитый в PRD.
...
До вызова подсказки никакие клетки target-пути, включая стартовую, не подсвечиваются пассивно.
...
Подсказка и ручная пересборка уровня с первого использования являются платной помощью.

В v1.1 под “платной” помощью понимается rewarded ad на каждое применение.

Бесплатного help-token / free-action timer в продукте больше нет.

Автопересборка после прохождения уровня не является help action и не монетизируется.

13.2 Help Buttons UX States
State A — Ad-required

Кнопка в обычном активном состоянии

маркер `free` / badge не показывается

State B — Disabled / Loading
...
Переход на 6×6 и увеличение доли длинных слов повышают риск ухудшения читаемости и touch-accuracy на маленьких экранах

нужен отдельный контроль на mobile playtest
...
acceptable для v1 при сохранении очков
...
UI не должен раскрывать несуществующую free-экономику; help-кнопки сразу работают как paid help
...
Никакая часть пути слова не должна подсвечиваться до явного вызова hint.
