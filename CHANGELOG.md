...
- В словарь `data/dictionary.csv` добавлено слово `горох` (`type=noun`), чтобы оно валидно распознавалось как bonus-слово при свайпе по корректному пути.
- После обновления словаря полный baseline остаётся зелёным:
  - `npm run ci:baseline` — passed.
...
- `src/adapters/InputPath/index.ts` усилен алгоритм распознавания жеста:
  - добавлен directional snapping по вектору движения с угловой толерантностью для диагональных переходов;
  - добавлен hysteresis для стабилизации выбранного направления между соседними выборками pointer-движения;
  - добавлена интерполяция между pointer-событиями (`createInterpolatedPointerSamples`), чтобы не терять промежуточные клетки при редких/шумных событиях;
  - добавлен фильтр согласованности шага с фактическим направлением pointer-движения, чтобы избежать ложных осевых отклонений после диагонального snap.
- Расширено тестовое покрытие `tests/input-path.adapter.test.ts`:
  - добавлен регресс на off-axis диагональный жест (прежний дефект со схлопыванием в вертикаль/горизонталь);
  - добавлен тест sparse pointer-событий с сохранением диагональной цепочки.
- Обновлены артефакты проекта:
  - `BACKLOG.md`: задача `[CODE]-[010]` отмечена как выполненная;
  - добавлены `ADR/ADR-035-diagonal-swipe-snapping-code-010.md` и `tasks/CODE-010.md`.
- Верификация:
  - `npm run ci:baseline` — passed;
  - Playwright smoke (`$WEB_GAME_CLIENT`) — passed, артефакты: `output/web-game-code010-smoke/shot-0.png`, `shot-1.png`, `state-0.json`, `state-1.json`; `errors-*.json` отсутствуют.
