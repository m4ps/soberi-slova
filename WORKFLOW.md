...
10. Порядок исполнения задач определяется только доской Linear. Не используй `BACKLOG.md` для выбора, проверки или блокировки очередности.
11. Следующая задача может стартовать только после перевода текущей задачи в `Done` и только когда `SOURCE_REPO_PATH` находится на актуальном `main`, где `HEAD` совпадает с `origin/main`.
...
4. Текущая задача Linear и её описание для актуального scope и порядка выполнения.
...
3. Не используй `BACKLOG.md` как источник очередности задач и не блокируй запуск из-за содержимого backlog.
4. Следуй принципам DRY, KISS и Clean Code. Держи изменения минимальными, удобными для review и строго связанными с scope текущей задачи.
5. Если по ходу работы обнаружится необходимый follow-up вне текущего scope, опиши его в финальном сообщении как отдельную следующую задачу.
6. Сохраняй границы архитектуры (см. `tests/architecture-boundaries.test.ts`).
7. Перед передачей результата все тесты репозитория должны проходить. Если встречается падающий тест, исправь его, а не оставляй suite в красном состоянии.
8. Перед переводом задачи в `In Review` запусти `npm run ci:baseline`.
9. После завершения обнови `CHANGELOG.md` на русском языке.
10. Если реализация требует или вводит значимое решение, добавь ADR на русском языке в `ADR/`.
11. В финальном сообщении опиши, что было изменено, что было проверено и какие риски или блокеры остались.
