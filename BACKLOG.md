...
- [x] [CODE]-[010] Улучшить диагональный свайп: tolerance к неточному движению
Task Context: Исправь текущую проблему выбора диагонали, когда неточный жест схлопывается в вертикаль/горизонталь; добавь directional snapping с угловой толерантностью, hysteresis и интерполяцию между pointer-событиями, чтобы диагональная соседняя клетка выбиралась стабильно даже при шумном input.
Task DOD: На реальных и тестовых жестах диагональные переходы выбираются предсказуемо; точность диагонального ввода заметно выше без регрессий обычных горизонтальных/вертикальных свайпов и tail-undo.

- [x] [CODE]-[011] Добавить dev-only панель с целевыми словами уровня
...
- [ ] [CODE]-[012] Ослабить зависимость от конкретного пути target-слова (order-first validation)
Task Context: Убедись, что для target-слов важен правильный порядок букв, а не единственный заранее зафиксированный маршрут генератора; если слово собрано валидным путем по правилам соседства, оно должно засчитываться как target независимо от конкретной геометрии пути.
Task DOD: Одно и то же target-слово можно собрать несколькими допустимыми маршрутами; классификация, прогресс и начисление очков идентичны для всех валидных путей.
...
- [x] [CODE]-[014] Пофиксить незачет target-слов при реальной разгадке в UI
...
- [ ] [TEST]-[008] Добавить регрессионные тесты неточного диагонального свайпа
Task Context: Добавь unit/integration и Playwright-сценарии с off-axis диагональными жестами (с дрейфом курсора), чтобы гарантировать выбор диагональной клетки вместо ложного перехода по вертикали/горизонтали.
...
Task DOD: Валидные bonus-слова стабильно распознаются и скорятся по контракту; после completion бонусы не начисляются; повторы и несловарные слова не изменяют state.

- [ ] [TEST]-[011] Протестировать dev-debug панель target-слов и ее изоляцию от production
Task Context: Добавь проверки видимости debug-панели только в `dev` профиле, корректности списка слов и отсутствия панели/связанных утечек в production-сборке.
Task DOD: Dev-панель доступна и актуальна в debug-контуре; production UI/бандл не содержит debug-компонента.
